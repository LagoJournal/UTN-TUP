...
# Dataset: Nivel_Experiencia (ordinal), Tickets_Soporte (discreta),
...
archivo <- file.choose() # elegir "Datos Practica Tabla de Frecuencias en R (V Ordinal).xlsx"
datos <- read_excel(archivo)

# 1) Nivel_Experiencia - ORDINAL -> conservar orden Junior < SemiSenior < Senior
...
tabla_nivel <- table(datos$Nivel_Experiencia)
f_acum_nivel <- cumsum(tabla_nivel)
f_rel_nivel <- prop.table(tabla_nivel)
f_rel_acum_nivel <- cumsum(f_rel_nivel)

tabla_experiencia <- data.frame(
  Nivel = names(tabla_nivel),
  Frec = as.vector(tabla_nivel),
  Frec_Acum = as.vector(f_acum_nivel),
  Frec_Rel = round(as.vector(f_rel_nivel), 3),
  Frec_Rel_Acum = round(as.vector(f_rel_acum_nivel), 3)
)
print(tabla_experiencia, row.names = FALSE)
